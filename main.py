...
def run_post(slot='morning'):
    ...
    try:
        ...
        video_path = generate_reel(post_data)
        log.info(f'Video ready: {video_path}')
        result = post_reel_to_instagram(
            video_path, post_data['caption'],
...
def scheduler_loop():
    ...
    while True:
        ...
        # Slot-based keys prevent double-posting and survive across the full minute window
        ...
        time.sleep(20)  # Check every 20s so we never miss a minute window


def safe_scheduler():
    try:
        time.sleep(3)  # Let Flask/gunicorn finish binding first
        scheduler_loop()
    except Exception as e:
...
# ── Start scheduler at module load so it runs under gunicorn too ──
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port)import os, logging, time, traceback, threading
from datetime import datetime
from flask import Flask, jsonify
import pytz
...
def test_post():
    ...
    t = threading.Thread(target=run_post, kwargs={'slot':'morning'}, daemon=True)
    t.start()
    return jsonify({'status':'started','message':'Post generating in background. Check logs and Instagram in ~2 min!'}), 200

@app.route('/health')
def health():
    return jsonify({'status':'running','timezone':TZ,'morning':TIME_MORNING,'evening':TIME_EVENING}), 200
...
def safe_scheduler():
    try:
        ...
        while True:
            ...
            if hhmm == TIME_MORNING and key not in posted_today:
                posted_today.add(key)
                run_post('morning')
            elif hhmm == TIME_EVENING and key not in posted_today:
                posted_today.add(key)
                run_post('evening')
            if hhmm == '00:01':
                posted_today = set()
...
if __name__ == '__main__':
    t = threading.Thread(target=safe_scheduler, daemon=True)
    t.start()
    port = int(os.getenv('PORT', 5000))
    log.info(f'WBG Auto-Poster starting on port {port}')
